...
Support is a tedious but required task, which allows us to learn from and
understand how our users are consuming our products. While users are often
...
Innovation Days project.

## The Idea
Anders Elton had an idea he put on the Innovation Days board: create a bot
...
response from the bot, by using a Slack reaction, this allows us to understand
...
We also prioritize official Nova documentation over Slack history, the indexed
conversations serve as supplementary context for questions the docs doesn't
cover well.
...
We also index historical Slack conversations from the support channel. A Cloud
Run Job runs daily at 6 AM, fetching recent messages and loading them into a
...
The whole system runs on Cloud Run, keeping costs low (we only pay when handling
requests) and scaling automatically. Two Cloud Run Jobs handle the data
